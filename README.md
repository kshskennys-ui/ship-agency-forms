...
云端部署时，海事局查询站点可能拒绝云服务器的自动化访问，因此核验改为调用用户电脑上的本地助手：

1. 在当前航次工具的“船员名单”区域点击“下载核验工具”，安装 `ShipAgencySeafarerAgentSetup.exe`。
2. 安装程序会把助手加入当前 Windows 用户的启动项。首次安装后也可以直接打开助手，或重新打开系统网页。
3. 点击“检测本地工具”，确认显示“本地工具已连接”，再点击“核验中国籍海员证”。
4. 查询浏览器会在用户电脑上可见运行，结果逐人回传云端并保存到当前航次。

本地助手只监听 `127.0.0.1:17321`，不对局域网或互联网开放；它不保存业务数据库，也不会把船员名单主动发送给第三方，只在用户点击核验后访问海事局页面并把结果回传当前系统。安装包需要发布为 GitHub Release 资产 `ShipAgencySeafarerAgentSetup.exe`，网页上的下载链接才会生效。
